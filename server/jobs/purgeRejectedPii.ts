 ...
 */
/**
 * เงื่อนไขว่าแถวนั้นยังถือข้อมูลส่วนตัวอยู่ไหม
 *
 * ส่งออกไปให้หน้ารายงานใช้ด้วย เพื่อให้ตัวเลขที่แสดงตรงกับสิ่งที่งานล้าง
 * จะลงมือทำจริง ถ้าสองที่นิยามไม่เหมือนกัน หน้ารายงานจะบอกตัวเลขที่ไม่มีอยู่จริง
 */
export const STILL_HAS_PII = PURGED_FIELDS.filter((f) => f !== 'payload')
  .map((f) => `${f} IS NOT NULL`)
