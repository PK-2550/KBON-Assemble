  ...
  asyncHandler(async (_req, res) => {
    const { rows } = await pool.query(
      `SELECT code, tier, name, name_th, requires_expiry, sort_order
        ...
        WHERE active
          -- ใบระดับการขนส่งรายเที่ยว (PHYTO) เลือกได้แล้วตั้งแต่ 016
          -- ซึ่งผ่อนกฎที่บังคับให้ต้องมี shipment_ref
          --
          -- ใบพวกนี้ไม่ขึ้นเป็นตราสาธารณะบนหน้ารายชื่อฟาร์ม เพราะไม่ใช่
          -- คุณสมบัติถาวรของสวน ขาอ่านกรอง tier <> shipment ออกไว้แล้ว
          -- เก็บไว้เป็นประวัติให้เจ้าของสวนกับผู้ดูแลดูเท่านั้น
